...
def match(template, AIStr):
    ...
    """
    if isinstance(template, str) and isinstance(AIStr, str):
        try:
            return re.match(AIStringToRegex(template), AIStr).groupdict()
        except AttributeError:  # re.match() returned None
            return None
    else:
        # Return None if template or AIStr is not a string, skipping matching
        return None
